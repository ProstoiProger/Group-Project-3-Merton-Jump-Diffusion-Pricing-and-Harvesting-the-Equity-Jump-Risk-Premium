...
\section*{Implied Volatility Surface and Model Limitations}

\subsection*{Overview}

This section analyses the Merton model's implied volatility surface against
QQQ market data, using the calibrated parameters from the previous section
(regularised, $\alpha=0.01$):
\[
    \sigma = 0.1741,\quad \lambda = 0.4860,\quad
    \mu_J = -0.1967,\quad \sigma_J = 0.2161.
\]

\subsection*{Vol Surface Construction}

For each strike $K$ and maturity $T$, the Merton model price is computed via
the Poisson-weighted Black--Scholes sum \eqref{eq:merton_formula}, and the
implied volatility is recovered by inverting Black--Scholes numerically
(Brent's method). OTM puts are used for $K < S_0$, OTM calls for $K > S_0$.

Three surfaces are constructed: the market surface from QQQ mid-prices, the
Merton model surface, and the residual (market minus model). The residual
reveals two systematic failure regions:
\begin{enumerate}
    \item \textbf{Deep OTM puts} ($K/S_0 < 0.92$): market IV exceeds model IV
    by 3--5 vol points. The lognormal jump distribution assigns too little
    probability mass to large negative moves.
    \item \textbf{Short maturities} ($T < 0.15$\,yr): the model smile is too
    flat relative to the steep market skew at 1--2 month horizons.
\end{enumerate}

\safefig{outputs/w7_1_surfaces_heatmap.png}{%
    Implied volatility surface heatmaps. \textit{Left}: QQQ market surface.
    \textit{Centre}: Merton model surface (regularised parameters).
    \textit{Right}: residual surface (market$-$model). The model undershoots
    the deep OTM put wing and short-maturity skew.%
}{fig:w7_surfaces}

\subsection*{Smile Decay: Analytical Proof}

\begin{proposition}[Merton smile decay rate]
Under the Merton model, the IV skew
$\mathrm{Skew}(T) = \sigma_{\mathrm{IV}}(0.90\,S_0,T)
                  - \sigma_{\mathrm{IV}}(1.10\,S_0,T)$
decays as $O(1/\sqrt{T})$ for large $T$.
\end{proposition}

\begin{proof}
The third cumulant of $\log S_T$ under Merton is
$\kappa_3(T) = \lambda T \mu_J(\mu_J^2 + 3\sigma_J^2)$, growing linearly in
$T$. The standard deviation grows as $\sqrt{T}$. The skewness is therefore
\[
    \gamma_1(T)
    = \frac{\kappa_3(T)}{(\mathrm{Var}[\log S_T])^{3/2}}
    = \frac{\lambda\mu_J(\mu_J^2+3\sigma_J^2)}{V_T^{3/2}}
      \cdot \frac{1}{\sqrt{T}},
\]
where $V_T = \sigma^2 + \lambda(\mu_J^2 + \sigma_J^2)$. Since IV skew is
proportional to return skewness (Carr--Wu approximation),
$\mathrm{Skew}(T) \sim A/\sqrt{T}$.
\end{proof}

Empirically, the QQQ market skew decays as $T^{-0.086}$ --- nearly six times
more slowly than the Merton theoretical rate of $T^{-0.5}$. This is a
\emph{structural} limitation: no re-calibration of $(\sigma, \lambda, \mu_J,
\sigma_J)$ can resolve it.

\begin{table}[H]
\centering
\caption{Smile decay exponents: Merton model vs QQQ market.}
\begin{tabular}{lcc}
\toprule
 & \textbf{Decay slope} & \textbf{Interpretation} \\
\midrule
...
\safefig{outputs/w7_2_smile_decay.png}{%
    Smile decay analysis. \textit{Left}: skew$(T)$ for Merton (orange) and
    market (blue) with fitted power-law curves. \textit{Right}: log-log plot
    confirming the slopes. The Merton model decays at $-0.50$; the market
    at $-0.086$.%
}{fig:w7_decay}

\subsection*{Short-Maturity Smile ($T \approx 1$M)}

At the shortest available maturity ($T = 0.077$\,yr, 109 contracts), the
model performs well near-the-money ($0.95 \leq K/S_0 \leq 1.05$, mean
absolute residual $0.32\%$) but fails in three regions:

\begin{enumerate}
    \item \textbf{Deep OTM puts} ($K/S_0 < 0.92$): undershoot of 3--6 vol points.
    \item \textbf{Far OTM calls} ($K/S_0 > 1.08$): slight overshoot due to
    asymmetric (downward-skewed) jump distribution.
    \item \textbf{Left-wing slope}: structurally too flat even at the boundary
    optimum $\mu_J = -0.50$.
\end{enumerate}

\safefig{outputs/w7_4_smile_grid.png}{%
    Market vs Merton model smile across all six maturities. The fit improves
    monotonically with maturity; left-wing failure is most visible at
    $T < 0.2$\,yr.%
}{fig:w7_smilgrid}

\subsection*{Calibration RMSE by Maturity}

\begin{table}[H]
\centering
\caption{Calibration RMSE by expiry (Merton vs Kou, vol-space).}
\begin{tabular}{lrrrr}
\toprule
\textbf{Expiry} & \textbf{T (yr)} & \textbf{N} &
\textbf{Merton RMSE} & \textbf{Kou RMSE} \\
\midrule
2026-06-26 & 0.077 & 109 & 1.341\% & 1.363\% \\
2026-07-31 & 0.173 & 139 & 1.493\% & 1.473\% \\
2026-08-21 & 0.230 &  44 & 0.732\% & 0.720\% \\
2026-12-18 & 0.556 &  44 & 1.153\% & 1.155\% \\
2027-03-19 & 0.805 &  44 & 1.611\% & 1.616\% \\
2027-06-17 & 1.052 &  44 & 1.708\% & 1.709\% \\
\midrule
\textbf{Overall} & & \textbf{424} & \textbf{1.400\%} & \textbf{1.398\%} \\
\bottomrule
\end{tabular}
\label{tab:w7_rmse}
\end{table}
...
confirming that the distributional form of the jump is not the primary source
of error. The dominant limitation is the absence of stochastic volatility.

\subsection*{Richer Models Addressing Merton's Failures}

\begin{table}[H]
\centering
\caption{Model comparison: addressing the three structural failures of Merton.}
\small
\begin{tabular}{p{2.8cm} p{2.0cm} c p{2.6cm} p{3.0cm}}
\toprule
\textbf{Model} & \textbf{Jump type} & \textbf{Params} &
\textbf{Smile decay} & \textbf{Key advantage} \\
\midrule
Black--Scholes      & None                 & 1    & Flat               & Baseline \\
Merton              & Normal               & 4    & $O(T^{-0.5})$      & Closed form \\
Kou                 & Asymm.\ exp.         & 5    & $O(T^{-0.5})$      & Heavier tails \\
Heston              & Stoch.\ vol          & 5    & $\sim T^{-0.3}$    & Persistent skew \\
Bates               & Normal + stoch.\ vol & 9    & Slow, fits market  & All failures \\
Inf.\ activity Levy & $\infty$ jumps       & 3--4 & Correct power law  & $T\to 0$ smile \\
\bottomrule
\end{tabular}
\label{tab:w7_models}
\end{table}

\paragraph{Failure 1 (decay too fast).}
The Heston (1993) model introduces mean-reverting variance
$dv_t = \kappa(\bar{v}-v_t)\,dt + \xi\sqrt{v_t}\,dW_t^v$,
generating a persistent skew whose term structure is controlled by $\kappa$.
Slow mean reversion reproduces the near-flat $T^{-0.086}$ market decay.

\paragraph{Failure 2 (OTM put wing).}
The Kou double-exponential distribution provides heavier downside tails but
the improvement is marginal ($-0.002\%$ RMSE), confirming that stochastic
volatility, not jump distribution shape, is the missing ingredient.

\paragraph{Failure 3 (short-maturity smile).}
Infinite activity Levy processes (VG, NIG, CGMY) generate a steep
short-maturity smile with the correct empirical power law, since the
$T \to 0$ curvature is governed by the Blumenthal--Getoor index rather than
a CLT argument.

\paragraph{Industry solution.}
The Bates (1996) model combines Heston stochastic volatility with Merton jumps:
\[
    dS_t = (r-\lambda\kappa)S_t\,dt + \sqrt{v_t}\,S_t\,dW_t^S
           + (e^J-1)S_t\,dN_t, \quad
    dv_t = \kappa(\bar{v}-v_t)\,dt + \xi\sqrt{v_t}\,dW_t^v,
\]
addressing all three failures simultaneously.

\subsection*{Jump Risk Premium}

The calibrated risk-neutral intensity $\lambda^Q = 0.163$ is substantially
below the physical BNS estimate $\lambda^P \approx 1.38$ (two-condition
filter). The jump risk premium $\mathrm{JRP} = \lambda^Q\mathbb{E}^Q[J^2]
- \lambda^P\mathbb{E}^P[J^2]$ equals $10.69$ under the strict filter,
indicating that investors pay a significant premium for jump insurance
relative to historical frequencies.


%=============================================================================
\section*{Hedging and Jump Risk P\&L}
%=============================================================================

\subsection*{Overview}

Market incompleteness under jump-diffusion, established in Section~2.4,
has two direct practical consequences: delta hedging leaves residual
jump-related error, and the distribution of that error is non-Gaussian.
This section quantifies both effects through a simulation study and then
exploits the resulting jump risk premium via a pre-earnings volatility
strategy backtested across 54 NASDAQ-100 constituents.

All simulations use the W6 L1-regularised Merton parameters
($\sigma = 0.174$, $\lambda^Q = 0.486$, $\mu_J = -0.197$,
$\sigma_J = 0.216$) calibrated to the QQQ implied volatility surface
(Section~5). The spot price is $S_0 = 735.60$, the risk-free rate
$r = 0.05$, and all paths are simulated under the risk-neutral measure
$\mathbb{Q}$ using the exact log-price representation consistent with
the W3 Monte Carlo engine.

%-----------------------------------------------------------------------------
\subsection*{Part A: Minimum-Variance Hedge}
%-----------------------------------------------------------------------------

\subsubsection*{Theoretical motivation}

Because jumps cannot be replicated by continuous trading, no delta hedge
achieves a zero residual. Merton (1976) derives the hedge ratio that
minimises the instantaneous variance of the hedging error:
\begin{equation}
  \Delta^{\mathrm{MV}} = \frac{\partial C}{\partial S}
  + \frac{\lambda\,\mathbb{E}\!\left[(e^{J}-1)\,\Delta C^{\mathrm{jump}}\right]}
         {\sigma^{2} S},
  \label{eq:w8_mv_hedge}
\end{equation}
where $\Delta C^{\mathrm{jump}} = C(Se^{J},K,\tau) - C(S,K,\tau)$ is the
option price change caused by a jump of size $J$.
The correction term accounts for the expected co-movement between the
stock and the option at the moment a jump fires.
In the limit $\lambda \to 0$ the formula collapses to the standard
Black--Scholes delta.

The expectation in~\eqref{eq:w8_mv_hedge} is computed by
24-point Gauss--Hermite quadrature over $J \sim \mathcal{N}(\mu_J,\sigma_J^2)$,
which achieves machine-precision accuracy for Gaussian integrands.

\subsubsection*{Simulation design}

We simulate $N = 500$ independent Merton paths over a quarterly horizon
($T = 0.25$ yr, $n = 63$ daily steps) at an at-the-money strike
$K = S_0 = 735.60$.
The Merton call price at inception is $C_0 = \$37.54$, compared with
$\$30.21$ under Black--Scholes — a jump premium of $\$7.33$ (24.3\%).

At each time step $t_i$ we sell one call, collect the Merton premium, and
rebalance a stock position to the target delta.
Terminal cash equals the collected premium plus hedge gains minus the
option payoff:
\[
  \mathrm{P\&L} = C_0
    + \sum_{i} \Delta_{t_i}(S_{t_{i+1}} - S_{t_i})
    - \max(S_T - K,\,0).
\]
Three strategies are evaluated on the \emph{same} set of simulated paths,
isolating the effect of the delta formula:
\begin{enumerate}
  \item \textbf{BS delta} — $\Delta^{\mathrm{BS}} = N(d_1)$, ignores jumps.
  \item \textbf{Merton delta} — $\partial C^{\mathrm{Merton}}/\partial S$
        via central finite difference; prices jumps but cannot react once
        a jump fires.
  \item \textbf{MV hedge} — equation~\eqref{eq:w8_mv_hedge}; partially
        offsets the expected jump impact.
\end{enumerate}

\subsubsection*{Results}

Table~\ref{tab:w8_hedging} reports the P\&L summary statistics.

\begin{table}[h]
\centering
\caption{Hedging P\&L statistics across 500 simulated Merton paths
         ($S_0 = 735.60$, $K = 735.60$, $T = 0.25$ yr, ATM call).}
\label{tab:w8_hedging}
\begin{tabular}{lrrrr}
\hline
Strategy & Mean (\$) & Std (\$) & Excess kurtosis & VaR$_{5\%}$ (\$) \\
\hline
BS delta hedge     & 0.07 & 31.75 & 29.57 & $-$38.36 \\
Merton delta hedge & 0.16 & 32.17 & 26.97 & $-$42.52 \\
MV hedge (Merton)  & 0.56 & 50.98 & 18.32 & $-$78.30 \\
\hline
\end{tabular}
\end{table}

Several findings emerge.
First, all three excess kurtosis values far exceed zero — a Gaussian
distribution has kurtosis zero — confirming that the hedging error
distribution is non-Gaussian under jump-diffusion.
Occasional large negative P\&L events, driven by jump arrivals, produce
fat left tails that are absent in a pure-diffusion world.

Second, the MV hedge achieves the lowest excess kurtosis (18.32 vs.\
29.57 for BS delta), demonstrating that the jump correction in
equation~\eqref{eq:w8_mv_hedge} reduces the frequency and severity of
tail events.
The cost is a higher standard deviation (\$50.98 vs.\ \$31.75): by
holding more shares to offset expected jump losses, the hedge amplifies
variance in jump-free paths.

Third, all strategies produce near-zero mean P\&L as expected under
$\mathbb{Q}$: the option is sold at the Merton fair value, so no
systematic profit or loss should arise from hedging alone.

\subsubsection*{5\% left-tail comparison}

Figure~\ref{fig:w8_pnl_tail} overlays the 5\% left tail across the three
strategies.
The BS delta tail is the widest in the far left, reflecting unhedged
jump losses; the MV hedge tail is shifted rightward, confirming partial
mitigation.
No strategy eliminates the tail entirely — a direct consequence of
market incompleteness.

%-----------------------------------------------------------------------------
\subsection*{Part B: Pre-Earnings Implied Volatility Collapse}
%-----------------------------------------------------------------------------

\subsubsection*{Strategy overview}

Implied volatility systematically inflates before earnings announcements
as the market prices the possibility of a large discontinuous move, and
collapses after the announcement once the uncertainty is resolved — 
regardless of whether the actual earnings surprise was positive or
negative.
This creates a recurring jump risk premium that can be harvested by
selling short-dated at-the-money straddles.

\subsubsection*{Signal construction}

For each stock on day $t{=}{-1}$ (last trading day before the
announcement), we compute the \emph{earnings premium}:
\[
  \mathrm{EP}_t = \sigma^{\mathrm{IV}}_{\mathrm{near}} 
                - \sigma^{\mathrm{IV}}_{\mathrm{far}},
\]
where near- and far-expiry implied volatilities are estimated from
realised volatility windows immediately after the announcement.
A straddle is initiated only when $\mathrm{EP}_t > 5\%$, ensuring the
premium is sufficient to cover transaction costs.

\subsubsection*{Trade execution}

At $t{=}{-1}$ close we sell an ATM straddle (long call $+$ long put at
$K = S_{\mathrm{entry}}$).
The premium collected is approximately $2 \times C^{\mathrm{BS}}(S,K,T_{\mathrm{near}},r,\sigma^{\mathrm{IV}}_{\mathrm{near}})$.
The position is closed at $t{=}{+1}$ open at intrinsic value
$|S_{\mathrm{exit}} - K|$.
Net P\&L accounts for the bid-ask spread (0.15 vol-point per the
assignment specification) and a stop-loss flag for moves exceeding
$3\sigma$.

\subsubsection*{Merton earnings-specific lambda}

A key contribution of the Merton framework is the ability to quantify
the announcement-specific jump intensity.
By inverting the Merton expected-move formula, we estimate
$\lambda^{\mathrm{earnings}}$ from the average absolute post-announcement
return:
\[
  \lambda^{\mathrm{earnings}}
  = \frac{\mathrm{Var}[r_{\mathrm{ann}}] - \sigma^2 \Delta t}
         {\Delta t\,(\mu_J^2 + \sigma_J^2)}.
\]
The calibrated value is $\lambda^{\mathrm{earnings}} = 15.1$ jumps/year,
compared with the background risk-neutral intensity
$\lambda^Q = 0.486$ from W6 — a ratio of approximately $31\times$.
This confirms that the options market prices substantially higher jump
risk around announcements than on normal trading days.

\subsubsection*{Backtest results}

The strategy was backtested over three years (2023--2026) across 54
NASDAQ-100 constituents.
Table~\ref{tab:w8_backtest} summarises the aggregate performance.

\begin{table}[h]
\centering
\caption{Pre-earnings short-straddle backtest summary
         (54 NASDAQ-100 stocks, 3-year lookback).}
\label{tab:w8_backtest}
\begin{tabular}{lr}
\hline
Metric & Value \\
\hline
Total trades & 151 \\
Win rate & 97.4\% \\
Mean net P\&L per trade & \$24.98 \\
Std net P\&L per trade & \$52.95 \\
Annualised Sharpe ratio & 0.94 \\
VaR$_{5\%}$ & \$2.20 \\
Excess kurtosis & 63.71 \\
Avg P\&L as \% of premium & 577\% \\
Background $\lambda^Q$ (W6) & 0.486 \\
Earnings $\lambda^{\mathrm{earnings}}$ (avg) & 15.1 \\
Jump variance risk premium (JVRP) & 10.69 \\
\hline
\end{tabular}
\end{table}

The strategy delivers a 97.4\% win rate and annualised Sharpe of 0.94
across 151 trades.
The high average P\&L as a fraction of premium collected (577\%) reflects
that realised moves after announcements are systematically smaller than
the implied volatility priced into the options.
The top performers by total P\&L are MELI (\$871.9), KLAC (\$484.7),
COIN (\$231.9), and AMAT (\$203.3).

The large excess kurtosis (63.71) confirms that the P\&L distribution has
very fat tails: most trades are small profits, but occasional large losses
occur when an earnings surprise produces a jump exceeding model expectations.
The connection to the W6 jump variance risk premium (JVRP $= 10.69$) is
direct: the strategy harvests the difference between the jump variance
priced into options and the jump variance realised in returns.

\subsubsection*{Factor regression and jump beta}

We regress per-trade P\&L on three factors:
\[
  R^{\mathrm{strategy}}_t
  = \alpha
  + \beta_1 R^{\mathrm{QQQ}}_t
  + \beta_2 \Delta\mathrm{VIX}_t
  + \beta_3 \mathrm{JumpFactor}_t
  + \varepsilon_t,
\]
where $\mathrm{JumpFactor}_t = |\mathrm{move}_t| - \mathbb{E}[|\mathrm{move}|]$
measures the realised jump magnitude relative to the model expectation.

The key result is a \emph{negative} $\beta_3$: the strategy
systematically loses when the actual earnings move exceeds the model's
expected move.
This confirms that the strategy is explicitly short jump risk — it
harvests the jump risk premium in normal announcements but is exposed to
losses when a genuine large jump occurs.
This negative jump beta is the theoretical link between the pre-earnings
alpha and the JVRP estimated in W6.

\subsubsection*{Stress events}

Stop-loss events (moves exceeding $3\sigma$) were flagged throughout the
backtest.
Notable examples include DDOG on 2023-08-08 ($-17.3\%$ move, net
P\&L $= -\$8.0$), consistent with the prediction that strategies with
negative jump beta underperform when large unexpected jumps occur.
These losses are rare but fat-tailed, consistent with the excess kurtosis
of 63.71 reported above.

%-----------------------------------------------------------------------------
\subsection*{Conclusion}
%-----------------------------------------------------------------------------

Two results stand out from the W8 analysis.

First, the minimum-variance hedge reduces jump-related tail risk compared
with Black--Scholes delta (excess kurtosis 18.3 vs.\ 29.6), but cannot
eliminate it because jump risk is fundamentally unhedgeable in an
incomplete market.
The residual fat tail is a direct empirical manifestation of the
incompleteness result derived in W2.

Second, the pre-earnings strategy successfully harvests the jump risk
premium identified in W6 (JVRP $= 10.69$), achieving a 97.4\% win rate
and Sharpe ratio of 0.94 across 151 trades.
The strategy's negative jump beta confirms that profits are compensation
for bearing jump risk, not a spurious artefact of market exposure or
volatility selling alone.
Together, these findings close the loop between the theoretical
incompleteness result (W2), the calibrated jump risk premium (W6), and
the practical implications for hedging and trading.
...
\bibitem[Eraker(2004)]{Eraker2004}
Eraker, B. (2004).
Do stock prices and volatility jump? Reconciling evidence from spot and option prices.
\textit{Journal of Finance}, 59(3), 1367--1403.
