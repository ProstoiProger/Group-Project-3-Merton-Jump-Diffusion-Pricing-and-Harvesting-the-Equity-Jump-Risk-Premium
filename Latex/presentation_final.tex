...
\section{W8: Hedging \& Jump Risk P\&L}

% ── Slide 1: Part A overview ─────────────────────────────────────────────────
\begin{frame}{W8 Part A: Minimum-Variance Hedge}
\begin{columns}[T]
\column{0.50\textwidth}
\begin{block}{Why delta hedging fails under jumps}
\footnotesize
\begin{itemize}\itemsep3pt
  \item Under Black--Scholes: continuous rebalancing achieves \emph{zero} residual
  \item Under Merton: jump fires instantaneously — no rebalancing fast enough
  \item Incomplete market $\Rightarrow$ residual error is unavoidable
\end{itemize}
\end{block}
...
\[
  ...
  + \frac{\lambda\,\mathbb{E}[(e^{J}-1)\,\Delta C^{\mathrm{jump}}]}{\sigma^{2}S}
\]
Correction term offsets the \emph{expected} option price change when a jump fires.
Computed via 8-point Gauss--Hermite quadrature.
\end{block}

\column{0.50\textwidth}
\begin{block}{Three strategies compared}
\footnotesize
\begin{enumerate}\itemsep3pt
  \item \textbf{BS delta} — $N(d_1)$, ignores jumps
  \item \textbf{Merton delta} — $\partial C^{\mathrm{Merton}}/\partial S$, prices jumps
  \item \textbf{MV hedge} — equation above, corrects for jump impact
\end{enumerate}
\end{block}

\begin{alertblock}{Key setup}
\footnotesize
$S_0 = \$735.60$ (QQQ), $K = S_0$, $T = 0.25$\,yr\\
Merton $C_0 = \$37.54$ vs BS $C_0 = \$30.21$\\
\textbf{Jump premium: \$7.33 (24.3\%)}
\end{alertblock}
\end{columns}
\end{frame}

% ── Slide 2: Hedging results table ───────────────────────────────────────────
\begin{frame}{W8 Part A: Hedging P\&L Results}
\begin{columns}[T]
\column{0.55\textwidth}
\begin{block}{P\&L statistics (500 paths, ATM, $T=0.25$\,yr)}
\footnotesize
\begin{tabular}{lrrrr}
\toprule
Strategy & Mean & Std & Kurt. & VaR$_{5\%}$ \\
\midrule
BS delta    & 0.07 & 31.75 & \textbf{29.6} & $-$38.4 \\
Merton $\Delta$ & 0.16 & 32.17 & \textbf{27.0} & $-$42.5 \\
MV hedge    & 0.56 & 50.98 & \textbf{18.3} & $-$78.3 \\
\bottomrule
\end{tabular}
\end{block}

\begin{block}{Non-Gaussianity proof}
\footnotesize
Gaussian distribution has excess kurtosis $= 0$.\\
All three strategies show kurtosis $\gg 0$ $\Rightarrow$\\
\textbf{Fat tails confirmed under jump-diffusion.}\\[3pt]
MV hedge reduces kurtosis most (29.6 $\to$ 18.3)\\
but cannot eliminate tail risk — incompleteness.
\end{block}
...
\begin{itemize}\itemsep4pt
  \item \textbf{Mean $\approx 0$}: fair pricing under $\mathbb{Q}$
  \item \textbf{High std}: residual jump exposure
  \item \textbf{Kurtosis $\downarrow$}: MV hedge reduces tail events
  \item \textbf{VaR$_{5\%}$ worse for MV}: holds more shares to offset jumps, amplifies variance in jump-free paths
\end{itemize}
\end{block}

\begin{alertblock}{Practical implication}
\footnotesize No hedge eliminates jump risk.\\
This is the empirical consequence\\
of market incompleteness (W2).
\end{alertblock}
\end{columns}
\end{frame}

% ── Slide 3: Part B strategy ─────────────────────────────────────────────────
\begin{frame}{W8 Part B: Pre-Earnings IV Collapse Strategy}
\begin{columns}[T]
\column{0.50\textwidth}
\begin{block}{Economic intuition}
\footnotesize
\begin{itemize}\itemsep3pt
  \item Before earnings: IV inflates as market prices jump risk
  \item After announcement: uncertainty resolved, IV collapses
  \item This happens \emph{regardless} of whether news is good or bad
  \item $\Rightarrow$ Systematic opportunity to harvest jump risk premium
\end{itemize}
\end{block}

\begin{block}{Trade mechanics}
\footnotesize
\begin{enumerate}\itemsep2pt
  \item Compute $\mathrm{EP}_t = \sigma^{\mathrm{IV}}_{\mathrm{near}} - \sigma^{\mathrm{IV}}_{\mathrm{far}}$
  \item Trade only if $\mathrm{EP}_t > 5\%$
  \item $t{=}{-1}$: sell ATM straddle at close
  \item $t{=}{+1}$: close at open (post-announcement)
  \item Net P\&L after bid-ask + 0.15 vol-pt cost
\end{enumerate}
\end{block}

\column{0.50\textwidth}
\begin{block}{Merton earnings lambda}
\footnotesize
By inverting the Merton expected-move formula:
\[
  \lambda^{\mathrm{earnings}} = 15.1 \text{ jumps/yr}
\]
vs.\ background $\lambda^Q = 0.486$ (W6)\\[3pt]
\textbf{Ratio: $31\times$ higher around announcements}\\[3pt]
Options market prices 31$\times$ more jump risk at earnings than on normal days.
\end{block}

\begin{alertblock}{Universe}
\footnotesize
54 NASDAQ-100 stocks $\times$ 3 years\\
$= 151$ qualifying trades
\end{alertblock}
\end{columns}
\end{frame}

% ── Slide 4: Backtest results ─────────────────────────────────────────────────
\begin{frame}{W8 Part B: Backtest Results \& Jump Beta}
\begin{columns}[T]
\column{0.50\textwidth}
\begin{block}{Strategy performance}
\footnotesize
\begin{tabular}{lr}
\toprule
Metric & Value \\
...
Trades & 151 \\
Win rate & \textbf{97.4\%} \\
Mean P\&L / trade & \$24.98 \\
Sharpe (ann.) & \textbf{0.94} \\
VaR$_{5\%}$ & \$2.20 \\
JVRP (W6) & \textbf{10.69} \\
\bottomrule
\end{tabular}
...
\begin{block}{Top 5 tickers by P\&L}
\footnotesize
\begin{tabular}{lrr}
\toprule
Ticker & Trades & Total P\&L \\
\midrule
MELI & 3 & \$871.9 \\
KLAC & 6 & \$484.7 \\
COIN & 6 & \$232.0 \\
...
\column{0.50\textwidth}
\begin{block}{Factor regression}
\footnotesize
\[
  R^{\mathrm{strat}}_t = \alpha + \beta_1 R^{\mathrm{QQQ}}_t
  + \beta_2 \Delta\mathrm{VIX}_t + \beta_3 \mathrm{JumpFactor}_t
\]
\textbf{Key result: $\beta_3 < 0$ (negative jump beta)}\\[3pt]
The strategy loses when actual earnings move $>$ model expectation $\Rightarrow$ explicitly short jump risk.
\end{block}

\begin{alertblock}{Closing the loop}
\footnotesize
\begin{itemize}\itemsep2pt
  \item W2: jump risk is unhedgeable (incompleteness)
  \item W6: JVRP $= 10.69$ — investors overpay for jump insurance
  \item W8: harvest that premium via pre-earnings straddle
  \item Negative jump beta confirms the source of alpha
\end{itemize}
\end{alertblock}
\end{columns}
\end{frame}
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{W8: MV hedge + earnings alpha harvested}
\end{enumerate}
\end{block}
...
\begin{itemize}\itemsep2pt
  ...
  \item \textbf{MV hedge: kurtosis reduced 29.6 $\to$ 18.3}
  \item \textbf{Earnings backtest: 97.4\% win rate, Sharpe 0.94}
  \item \textbf{JVRP $= 10.69$ harvested via short straddle}
\end{itemize}
\end{block}

\begin{alertblock}{Central finding}
\footnotesize Jump risk is unhedgeable (W2) but \emph{priceable} (W6) and \emph{harvestable} (W8). The gap $\lambda^{\mathbb{Q}} - \lambda^{\mathbb{P}}$ manifests as a 97.4\% win-rate earnings premium across 151 trades.
\end{alertblock}
\end{columns}
\end{frame}
