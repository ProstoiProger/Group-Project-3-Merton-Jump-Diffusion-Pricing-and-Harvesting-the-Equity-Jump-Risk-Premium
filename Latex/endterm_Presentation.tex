...
\begin{frame}{COS Method: Spectral Convergence}
\begin{columns}[T]
\column{0.50\textwidth}
{\small\textbf{Fang \& Oosterlee (2008) formula:}}
\[
C_0 = e^{-rT}\sum_{k=0}^{N-1}{}' \,
\Re\!\bigl[\phi(u_k)\,e^{-iu_k a}\bigr]\,U_k
\]
{\footnotesize $U_k = \frac{2}{b-a}\bigl[\chi_k(c,b) - K\,\psi_k(c,b)\bigr]$}

\smallskip
{\footnotesize Helper integrals $\chi_k$, $\psi_k$ computed in closed form. Truncation: $a = \kappa_1 - L\sqrt{\kappa_2+\sqrt{|\kappa_4|}}$, $L=10$.}

\column{0.50\textwidth}
\begin{block}{Spectral convergence}
\footnotesize
\begin{tabular}{rc}
\toprule
$N$ & $|C^{\text{COS}}-C^{\text{exact}}|$ \\
\midrule
 16  & $3.7\times10^{-1}$ \\
 32  & $2.0\times10^{-3}$ \\
 64  & $9.5\times10^{-11}$ \\
...
\footnotesize $N=128$ reaches floating-point precision. Both Merton and Kou use the same COS engine --- only $\phi$ changes.
\end{alertblock}

\begin{block}{Digital options}
\footnotesize Cash-or-nothing and asset-or-nothing: swap $U_k$ coefficients. Parity checks pass to machine precision.
\end{block}
\end{columns}
\end{frame}

\begin{frame}{Merton vs.\ Kou: Implied Volatility Smiles}
\begin{columns}[T]
\column{0.50\textwidth}
\begin{block}{Merton smile}
\footnotesize
\begin{itemize}\itemsep2pt
  \item Approximately \textbf{symmetric} about ATM
  \item Log-normal jumps carry only mild negative skew
  \item Underfits the steep put wing of equity indices
\end{itemize}
\end{block}

\medskip
\begin{block}{Kou smile}
\footnotesize
\begin{itemize}\itemsep2pt
  \item Pronounced \textbf{negative skew}
  \item Heavier mean down-jump ($1/\eta_2$) inflates OTM put IV
  \item Better match to QQQ option surface
\end{itemize}
\end{block}

\column{0.50\textwidth}
\begin{block}{Parameter effects on the surface}
\footnotesize
...
\midrule
$\sigma$ & Level of IV \\
$\lambda$ & Amount of tail risk priced \\
$\mu_J < 0$ & Downside skew \\
$\sigma_J$ & Tail thickness / kurtosis \\
$p,\,\eta_1,\,\eta_2$ & Asymmetry (Kou only) \\
\bottomrule
\end{tabular}
\end{block}

\begin{alertblock}{Calibration strategy (W6)}
\footnotesize Fit Merton/Kou to QQQ option cross-section; compare $\lambda^{\mathbb{Q}}$ vs $\lambda^{\mathbb{P}}$ for the jump risk premium.
\end{alertblock}
\end{columns}
\end{frame}
...
\begin{columns}[T]
\column{0.50\textwidth}
{\small\textbf{Sample:} 2023-05-23 to 2026-05-20, $N=732$ trading days.}
...
\footnotesize Far heavier tails than the normal distribution. Directly motivates a jump component.
\end{alertblock}

\medskip
\begin{block}{Option data (2026-05-20)}
\footnotesize 901 contracts across 6 expiries after cleaning.\\
Criteria: live bid/ask \emph{or} stale last price with open interest.\\
Arbitrage flags: monotonicity + PCP checks.
\end{block}
\end{columns}
\end{frame}
...
\begin{block}{Lognormal jump-size fit}
\footnotesize $\hat\mu_J = -4.89$, $\hat\sigma_J = 1.16$\\
KS test $p = 0.118$ --- not rejected at 5\%.\\
Broadly consistent with Merton assumption.
\end{block}
\end{columns}
\end{frame}
